        ...
        return(
            <div className="concept-input" onMouseOver={this.showConfigButton} onMouseOut={this.hideConfigButton}>
                <div id={this.props.concept.keyword+this.props.id+'config-btn'} className="concept-config-btn hidden-button" >
                    <p>peso: </p>
                    <input type="number" step="0.1" value={this.state.weight} onChange={(event) => this.props.updateWeight(this.props.id, event.target.value)} />
                </div>
                ...
                <input id={this.props.id} onChange={(event) => {this.props.saveRegex(event, this.state.weight)}} type="text" />
            </div>
        )
    }
