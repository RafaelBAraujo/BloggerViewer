    ...
    static createClassSummarySheet(templateWorksheet, students, post, postId) {
        ...
        for(let rowNum = 1; rowNum < 2; ++rowNum) {
            for(let colNum = 5; colNum < concepts.length+5; ++colNum) {
                let concept = concepts[colNum-5].keyword
        ...
        Object.keys(students).forEach((studentId) => {
            ...
            if(typeof student.posts[postId] !== 'undefined') { // if student has any comments or is identified
                ...
                if(typeof studentComments !== 'undefined' && studentComments !== null) {
                    ...
                    Object.keys(studentComments).forEach((studentComment) => {
                        ...
                        let studentConcepts = studentComments[studentComment].keywordUsage.keywords
                        let conceptStartColumn = 5
                        let studentScore = 0
                        if(typeof studentConcepts !== 'undefined') {
    
                            studentConcepts.forEach((keywordUsage) => {
    
                                if(typeof conceptCount[studentConcepts.indexOf(keywordUsage)] === 'undefined') {
                                    conceptCount[studentConcepts.indexOf(keywordUsage)] = 0
                                }
    
                                if(keywordUsage.fulfill >= 1) {
                                    conceptCount[studentConcepts.indexOf(keywordUsage)]++
                                }
    
                                if(parseInt(keywordUsage.fulfill) == 1) {
                                    conceptUsageTotal += keywordUsage.fulfill
                                }
                                templateWorksheet[xlsx.utils.encode_cell({r: currentCell.r, c: conceptStartColumn})] = { v: keywordUsage.fulfill }
                                if(parseInt(keywordUsage.fulfill) == 1) {
                                    studentScore += keywordUsage.fulfill
                                }
                                conceptStartColumn++
